  ...
  try {
    ...
    if (!text) {
      await sock.sendMessage(from, { 
        text: `> SIGMA MDX DEPLOY : ?? Veuillez fournir un lien Instagram
> Exemple : .instagram https://www.instagram.com/p/XXXX`
    ...
    if (!instaPatterns.some(p => p.test(text))) {
      await sock.sendMessage(from, { text: "> SIGMA MDX DEPLOY: ? Lien Instagram invalide." }, { quoted: msg });
      return;
    }
    ...
    if (!downloadData?.data?.length) {
      await sock.sendMessage(from, { text: "> SIGMA MDX DEPLOY:? Aucun média trouvé ou le post est privé." }, { quoted: msg });
      return;
    }
    ...
    if (!mediaToDownload.length) {
      await sock.sendMessage(from, { text: "> SIGMA MDX DEPLOY:? Aucun média valide trouvé à télécharger." }, { quoted: msg });
      return;
    }
      ...
      try {
        if (isVideo) {
          await sock.sendMessage(from, {
            video: { url },
            mimetype: "video/mp4",
            caption: "> SIGMA MDX DEPLOY: ? Téléchargement réussi."
          }, { quoted: msg });
        } else {
          await sock.sendMessage(from, {
            image: { url },
