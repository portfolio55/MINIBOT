...
export async function execute(sock, msg, args) {
  ...
  try {
    ...
    if (owner) {
      ...
      creatorText = creatorInGroup ? `@${owner.split("@")[0]}` : "> SIGMA MDX DEPLOY ?? Créateur absent";
    } else {
      creatorText = "Non disponible";
    }

    // === Construction du message au style SIGMA MDX MD avec '>' devant chaque ligne ===
    let infoText = 
`> SIGMA MDX DEPLOY: ℹ️ Infos du groupe

> ?? Nom du groupe       : ${groupName}
> ?? Créateur            : ${creatorText}

> ?👑 Administrateurs     : ${adminMentions.join(", ")}
...
> Dev by SIGMA MDX`;
    ...
    await sock.sendMessage(from, { text: "> SIGMA MDX DEPLOY: ?? Impossible de récupérer les informations du groupe." });
  }
}