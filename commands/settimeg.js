  ...
  try {
    ...
    if (!input.includes("/")) {
      ...
      return await sock.sendMessage(from, {
        ...
        text: "> SIGMA MDX DEPLOY ?? Format invalide.\nExemple : `!settimeg 08:00/22:00`",
    ...
    if (!/^\d{2}:\d{2}$/.test(openTime) || !/^\d{2}:\d{2}$/.test(closeTime)) {
      ...
      return await sock.sendMessage(from, {
        ...
        text: "> SIGMA MDX DEPLOY :?? Utilise le format HH:mm (exemple : 08:00/22:00).",
    ...
    await sock.sendMessage(from, {
      ...
      text: `> SIGMA MDX DEPLOY ? Horaires configurés avec succés :\n\n?? Ouverture : ${openTime}\n?? Fermeture : ${closeTime}`,
    ...
    await sock.sendMessage(from, {
      ...
      text: "> SIGMA MDX DEPLOY: ? Une erreur est survenue lors de la configuration des horaires.",
    ...
    for (const groupId in groupSchedules) {
      ...
      try {
        ...
        if (now === schedule.open) {
          ...
          await sock.sendMessage(groupId, { text: "> SIGMA MDX DEPLOY :?? Le groupe est maintenant *ouvert* !" });
        ...
        if (now === schedule.close) {
          ...
          await sock.sendMessage(groupId, { text: "> SIGMA MDX DEPLOY :?? Le groupe est maintenant *fermé* !" });
