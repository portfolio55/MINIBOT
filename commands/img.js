...
export async function execute(sock, msg, args, prefix = ".") {
  ...
  try {
    ...
    // ?? Extraire le mot-clé et le nombre déimages
    ...
    await sock.sendMessage(from, {
      text: `> SIGMA MDX DEPLOY ${prefix}Recherche de *${count}* image(s) pour : *${query}*...\n> ❌ Veuillez patienter...`
    }, { quoted: msg });
    ...
    // ?? Extraire les URLs déimages valides
    ...
    if (sent === 0) {
      ...
      await sock.sendMessage(from, {
        text: `> SIGMA MDX DEPLOY: ${prefix}${sent}/${count} image(s) envoyée(s) pour *${query}*.`
      }, { quoted: msg });
