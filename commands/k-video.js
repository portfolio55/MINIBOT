  ...
  try {
    ...
    if (!prompt) {
      await sock.sendMessage(from, {
        text: `> SIGMA MDX DEPLOY :?? Fournis une description pour générer la vidéo.\n\n> Exemple : . k-video anime girl with short blue hair`
      }, { quoted: msg });
      return;
    }
...
> +---------------+`
    ...
    const { data } = await axios.get(apiUrl, {
      timeout: 120000,
      headers: { "User-Agent": "Mozilla/5.0 (SIGMA MDDEPLOY)" }
    });
    ...
    await sock.sendMessage(from, {
      video: { url: videoUrl },
      mimetype: "video/mp4",
      caption: `> ?? **Vidéo générée par SIGMA MDX DEPLOY*\n> ?? Objectif : ${prompt}`
    }, { quoted: msg });

  } catch (err) {
    console.error("? Erreur K12 :", err?.message || err);
    await sock.sendMessage(from, {
      text: `> ?? SIGMA MDX DEPLOY: échec de la génération vidéo.\n> Réessaie avec un autre prompt.`
    }, { quoted: msg });
  }
}