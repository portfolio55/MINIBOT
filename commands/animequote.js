  ...
  try {
    ...
    if (!data || !data.quote) {
      return await sock.sendMessage(
        jid,
        { text: "> SIGMA MDX DEPLOY ? Impossible de récupérer la citation." },
        { quoted: m }
      );
    }
    ...
    await sock.sendMessage(
      jid,
      {
        text: `⚡ SIGMA MDX DEPLOY Anime Quote:\n\n"${data.quote}"\né ${data.character} (${data.anime})`
      },
      { quoted: m }
    );
    ...
    await sock.sendMessage(
      m.key.remoteJid,
      { text: `? SIGMA MDX DEPLOY AnimeQuote Error: ${err.message}` },
      { quoted: m }
    );
  }
}