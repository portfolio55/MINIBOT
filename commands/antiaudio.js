...
export async function execute(sock, msg, args, from, botContext) {
  try {
    ...
    if (!arg || !["on", "off"].includes(arg)) {
      const current = _getGP(from).antiVoice ? "activé" : "désactivé";
      await sock.sendMessage(from, { 
        text: `> SIGMA MDX DEPLOY: Anti-Audio\n\nétat : ${current}\n\nUtilisation : \`!antiaudio on\` ou \`!antiaudio off\``
      }, { quoted: msg });
      return;
    }

    const newState = arg === "on";
    _setGP(from, "antiVoice", newState);

    await sock.sendMessage(from, { 
      text: `> SIGMA MDX DEPLOY: Anti-Audio ${newState ? "activé" : "désactivé"} dans ce groupe.`
    }, { quoted: msg });
