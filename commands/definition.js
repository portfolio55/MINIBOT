  ...
  try {
    if (!args.length) {
      await sock.sendMessage(from, { 
        text: "> ?? SIGMA MDX DEPLOY : Usage: !definition <mot>\nEx: !definition algorithm" 
      }, { quoted: msg });
      return;
    }
    ...
    if (data[0]) {
      const entry = data[0];
      let reply = `> ?? SIGMA MDX DEPLOY : ${entry.word}\n🎨🎨🎨🎨🎨🎨\n`;
      ...
      await sock.sendMessage(from, { text: "> ❌ SIGMA MDX DEPLOY : Mot non trouvé." }, { quoted: msg });
    }
    
  } catch (err) {
    console.error("Erreur definition :", err);
    await sock.sendMessage(from, { text: "> SIGMA MDX DEPLOY : Service dictionnaire indisponible." }, { quoted: msg });
  }
}