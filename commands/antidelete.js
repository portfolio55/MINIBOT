...
export async function execute(sock, msg, args, from, botContext) {
  try {
    ...
    if (!arg || !["on", "off"].includes(arg)) {
      const current = _getGP(from).antiDelete ? "activé" : "désactivé";
      await sock.sendMessage(from, {
        text: `> SIGMA MDX DEPLOY: Anti-Delete\n\nétat : ${current}\n\nRécupère les messages (texte/image/sticker) supprimés dans le groupe, tant qu'ils ont été reçus depuis que le bot est en ligne (fenêtre glissante, max 6h).\nUtilisation : \`.antidelete on\` ou \`.antidelete off\``
      });
      return;
    }

    const newState = arg === "on";
    _setGP(from, "antiDelete", newState);
