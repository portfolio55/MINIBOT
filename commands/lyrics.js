  ...
  try {
    ...
    if (!songTitle) {
      await sock.sendMessage(from, { 
        text: "> SIGMA MDX DEPLOY:?? Veuillez entrer le nom de la chanson pour obtenir les paroles !\n\nExemple : . lyrics Shape of You" 
      }, { quoted: msg });
      return;
    }
    ...
    if (!lyrics) {
      await sock.sendMessage(from, {
        text: `> SIGMA MDX MD:? aucune parole trouvée pour : "${songTitle}"`
      }, { quoted: msg });
      return;
    }

    // Limite é 4096 caractères
    const maxChars = 4096;
...
> +-----------------+`
    ...
    await sock.sendMessage(msg.key.remoteJid, {
      text: `> SIGMA MDX DEPLOY:? Une erreur est survenue lors de la récupération des paroles pour "${args.join(" ")}".`
    }, { quoted: msg });
  }
}