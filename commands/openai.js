...
export async function execute(sock, msg, args, from) {
  ...
  if (!query) {
    return await sock.sendMessage(from, {
      text: "> Fournis un message pour OpenAI.\nExemple : !openai Explique-moi le machine learning"
    }, { quoted: msg });
  }

  try {
    await sock.sendMessage(from, { text: "🧠 OpenAI reflechit..." }, { quoted: msg });

    const answer = await queryAI(query);
    if (!answer) {
      return await sock.sendMessage(from, {
        text: "> OpenAI n'a pas repondu. Reessaie plus tard."
      }, { quoted: msg });
    }
