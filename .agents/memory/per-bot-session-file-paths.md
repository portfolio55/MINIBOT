...
Related WhatsApp/Baileys quirk: modern WhatsApp accounts identify by LID, and mentions of a LID-based user appear in `contextInfo.mentionedJid` as `${lid}@lid`, NOT `${lid}@s.whatsapp.net`. When checking "did this message mention user X" against a saved LID, check the `@lid` suffix (and keep `@s.whatsapp.net` only as a fallback for phone-number-based JIDs).
